...
Standard TRACE daily bond pricing panel covering 2024-present, processed through the Stage 0 Dick-Nielsen cleaning pipeline with decimal-shift correction and bounce-back filtering. Data is stored as hive-partitioned Parquet files under `_data/stage0/standard/year=YYYY/month=MM/data.parquet`.

Standard TRACE covers bonds that are not eligible for Enhanced TRACE. It became available more recently and thus has a shorter history.
