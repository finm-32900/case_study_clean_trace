...
Rule 144A TRACE daily bond pricing panel covering 2002-present, processed through the Stage 0 Dick-Nielsen cleaning pipeline with decimal-shift correction and bounce-back filtering. Data is stored as hive-partitioned Parquet files under `_data/stage0/144a/year=YYYY/month=MM/data.parquet`.

Rule 144A bonds are privately placed securities that can only be traded among qualified institutional buyers. This dataset captures these transactions separately from Enhanced and Standard TRACE.
